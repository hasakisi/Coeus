...
if (!_default.autoReplaceSvg)
  _default.observeMutations = false;
var config$1 = _extends({}, _default);
WINDOW.FontAwesomeConfig = config$1;
function update(newConfig) {
  var params = arguments.length > 1 && arguments[1] !== void 0 ? arguments[1] : {};
  var _params$asNewDefault = params.asNewDefault, asNewDefault = _params$asNewDefault === void 0 ? false : _params$asNewDefault;
  ...
  Object.keys(newConfig).forEach(function(configKey) {
    if (ok(configKey))
      config$1[configKey] = newConfig[configKey];
  });
}
function auto(value) {
  update({
...
function joinStyles(styles2) {
  return Object.keys(styles2 || {}).reduce(function(acc, styleName) {
    return acc + (styleName + ": " + styles2[styleName] + ";");
  }, "");
}
...
function transformForSvg(_ref2) {
  ...
  var innerTranslate = "translate(" + transform2.x * 32 + ", " + transform2.y * 32 + ") ";
  var innerScale = "scale(" + transform2.size / 16 * (transform2.flipX ? -1 : 1) + ", " + transform2.size / 16 * (transform2.flipY ? -1 : 1) + ") ";
  var innerRotate = "rotate(" + transform2.rotate + " 0 0)";
  var inner = {
...
function transformForCss(_ref2) {
  ...
  if (startCentered && IS_IE) {
    val += "translate(" + (transform2.x / d - width / 2) + "em, " + (transform2.y / d - height / 2) + "em) ";
  } else if (startCentered) {
    val += "translate(calc(-50% + " + transform2.x / d + "em), calc(-50% + " + transform2.y / d + "em)) ";
  } else {
    val += "translate(" + transform2.x / d + "em, " + transform2.y / d + "em) ";
  }
  val += "scale(" + transform2.size / d * (transform2.flipX ? -1 : 1) + ", " + transform2.size / d * (transform2.flipY ? -1 : 1) + ") ";
  val += "rotate(" + transform2.rotate + "deg) ";
  return val;
}
var ALL_SPACE = {
  x: 0,
...
var makeIconMasking = function(_ref2) {
  var children = _ref2.children, attributes = _ref2.attributes, main = _ref2.main, mask = _ref2.mask, transform2 = _ref2.transform;
  var mainWidth = main.width, mainPath = main.icon;
  var maskWidth = mask.width, maskPath = mask.icon;
  var trans = transformForSvg({ transform: transform2, containerWidth: maskWidth, iconWidth: mainWidth });
  var maskRect = {
    tag: "rect",
...
var makeIconStandard = function(_ref2) {
  ...
  if (transformIsMeaningful(transform2)) {
    var trans = transformForSvg({ transform: transform2, containerWidth: main.width, iconWidth: main.width });
    children.push({
      tag: "g",
      attributes: _extends({}, trans.outer),
      children: [{
...
var asIcon = function(_ref2) {
  ...
  if (transformIsMeaningful(transform2) && main.found && !mask.found) {
    ...
    attributes["style"] = joinStyles(_extends({}, styles2, {
      "transform-origin": offset.x + transform2.x / 16 + "em " + (offset.y + transform2.y / 16) + "em"
    }));
  }
  return [{
    tag: "svg",
...
var asSymbol = function(_ref2) {
  ...
  return [{
    ...
    children: [{
      tag: "symbol",
      attributes: _extends({}, attributes, { id }),
      children
    }]
  }];
};
function makeInlineSvgAbstract(params) {
  var _params$icons = params.icons, main = _params$icons.main, mask = _params$icons.mask, prefix = params.prefix, iconName = params.iconName, transform2 = params.transform, symbol = params.symbol, title = params.title, extra = params.extra, _params$watchable = params.watchable, watchable = _params$watchable === void 0 ? false : _params$watchable;
  ...
  if (title)
    content.children.push({ tag: "title", attributes: { id: content.attributes["aria-labelledby"] || "title-" + nextUniqueId() }, children: [title] });
...
function makeLayersTextAbstract(params) {
  ...
  var styles2 = _extends({}, extra.styles);
  if (transformIsMeaningful(transform2)) {
    styles2["transform"] = transformForCss({ transform: transform2, startCentered: true, width, height });
...
function observe(options) {
  if (!MUTATION_OBSERVER)
    return;
  var treeCallback = options.treeCallback, nodeCallback = options.nodeCallback, pseudoElementsCallback = options.pseudoElementsCallback;
...
function findIcon(iconName, prefix) {
  ...
  if (iconName && prefix && styles[prefix] && styles[prefix][iconName]) {
    var icon2 = styles[prefix][iconName];
    var width = icon2[0];
    var height = icon2[1];
    var vectorData = icon2.slice(4);
    val = {
      found: true,
      width,
      height,
...
function generateSvgReplacementMutation(node, nodeMeta) {
  var iconName = nodeMeta.iconName, title = nodeMeta.title, prefix = nodeMeta.prefix, transform2 = nodeMeta.transform, symbol = nodeMeta.symbol, mask = nodeMeta.mask, extra = nodeMeta.extra;
  ...
  disableObservation(function() {
    toArray(root.querySelectorAll("*")).forEach(function(node) {
      [":before", ":after"].forEach(function(pos) {
        var styles2 = WINDOW.getComputedStyle(node, pos);
...
function onTree(root) {
  ...
  var prefixesDomQuery = ["." + LAYERS_TEXT_CLASSNAME + ":not([" + DATA_FA_I2SVG + "])"].concat(prefixes.map(function(p2) {
    return "." + p2 + ":not([" + DATA_FA_I2SVG + "])";
  })).join(", ");
...
var Library = function() {
  function Library2() {
    classCallCheck(this, Library2);
    this.definitions = {};
  }
  createClass(Library2, [{
    key: "add",
    value: function add() {
      var _this = this;
...
function findIconDefinition(params) {
  var _params$prefix = params.prefix, prefix = _params$prefix === void 0 ? "fa" : _params$prefix, iconName = params.iconName;
  if (!iconName)
    return;
...
function resolveIcons(next) {
  return function(maybeIconDefinition) {
    ...
    return next(iconDefinition, _extends({}, params, { mask }));
  };
}
...
var icon = resolveIcons(function(iconDefinition) {
  ...
  return apiObject(_extends({ type: "icon" }, iconDefinition), function() {
    ...
    return makeInlineSvgAbstract({
      ...
      prefix,
      iconName,
      transform: _extends({}, meaninglessTransform, transform2),
      symbol,
      title,
      extra: {
        attributes,
...
var text = function text2(content) {
  ...
  return apiObject({ type: "text", content }, function() {
    ensureCss();
...
function bootstrap() {
  ...
  namespace.hooks = _extends({}, namespace.hooks, {
    ...
    addShims: function addShims(shims2) {
      ...
      (_namespace$shims = namespace.shims).push.apply(_namespace$shims, toConsumableArray(shims2));
      build();
      autoReplace();
    }
  });
...
@fortawesome/fontawesome/index.es.js:
  (*!
